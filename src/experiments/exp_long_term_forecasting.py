        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                ...
                if (i + 1) % 100 == 0:
                    ...
                    # Record GPU memory usage if using GPU
                    if self.args.use_gpu and torch.cuda.is_available():
                        # Measure peak memory allocation for accurate GPU memory reporting
                        allocated_memory = torch.cuda.max_memory_allocated() / (
                            1024 * 1024 * 1024
                        )
                        print("allocated_memory:", allocated_memory)

                        # Reset peak stats for next measurement period
    ...
    def test(self, setting, test=0):
        ...
        if test:
            ...
            self.model.load_state_dict(
                torch.load(os.path.join("./temp/checkpoints/" + setting, "checkpoint.pth"))
            )

        preds = []
        trues = []
        folder_path = "./temp/test_results/" + setting + "/"
        ...
        # result save
        folder_path = "./temp/results/" + setting + "/"
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        ...
        if self.args.data == 'custom':
            ...
            if len(model_id_parts) >= 3 and model_id_parts[-2].isdigit() and model_id_parts[-1].isdigit():
        ...
        try:
            ...
            # Try to write without locking if fcntl is not available
            try:
                with open(output_file, "a") as f:
                    f.write(setting + "  \n")
                    f.write("mse:{}, mae:{}".format(mse, mae))
                    f.write("\n")
                    f.write("\n")
                print(f"Results written without lock to: {output_file}")
            except Exception as e2:
                print(f"ERROR: Still failed to write results: {str(e2)}")

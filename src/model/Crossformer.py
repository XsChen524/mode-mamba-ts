...
from src.layers.Crossformer_layer import TruncateModule, TSAEncoder, TSADecoder
...
class Model(nn.Module):
    def __init__(self, configs) -> None:
        ...
        assert (
            configs.embed_type == 1
        ), "Crossformer has decoders, Decoder Positional Embeddings Needed!"
        assert (
            configs.e_layers == configs.d_layers - 1
        ), "Crossformer uses cross-attention across encoders and decoders, must have another decoder for processing the initial state!"
        ...
        self.process_layer_enc = PatchEmbedding(
            self.patch_num,
            configs.d_model,
            configs.patch_len,
            configs.stride,
        ...
        if configs.seq_len % self.stride == 0:
            self.patch_num_out = int(
                (configs.pred_len - configs.patch_len) / configs.stride + 1
            )
        else:
            if configs.padding_patch == "end":
                self.patch_num_out = int(
                    (configs.pred_len - configs.patch_len) / configs.stride + 2
                )
            else:
                self.patch_num_out = int(
                    (configs.pred_len - configs.patch_len) / configs.stride + 1
        ...
        self.encoder = nn.ModuleList(
            [
                TSAEncoder(
                    ...
                    win_size=1 if i == 0 else 1 if not configs.distil else 2,
                    ...
                    patch_num=(
                        self.patch_num
                        if not configs.distil
                        else ceil(self.patch_num / 2**i)
                    ),
        ...
        self.D_P = nn.Parameter(
            torch.randn(1, configs.dec_in, self.patch_num_out, configs.d_model)
        )
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        output = torch.zeros(
            final_predict.shape[0], final_predict.shape[1], self.pred_len
        ).to(x_enc.device)

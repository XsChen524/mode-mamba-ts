...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        )  # q_s    : [bs x n_heads x channels x d_k]
        k_s = (
...
class TwoStageAttentionLayer(nn.Module):
    def __init__(
        ...
        if "Batch" in norm:
            self.norm_attn1 = nn.Sequential(
                transpose(1, 2), nn.BatchNorm1d(d_model), transpose(1, 2)
            )
            self.norm_ffn1 = nn.Sequential(
                transpose(1, 2), nn.BatchNorm1d(d_model), transpose(1, 2)
            )
    ...
    def forward(self, x):
        ...
        dim_send = torch.reshape(
            output, (-1, self.channels, output.shape[1], output.shape[2])
        ).permute(0, 2, 1, 3)
        ...
        output = output.reshape(
            self.batch_size, -1, output.shape[1], output.shape[2]
        ).permute(0, 2, 1, 3)
        output = output.reshape(
